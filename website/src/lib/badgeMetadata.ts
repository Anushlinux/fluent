...
/**
 * Generate placeholder badge image (fallback for ASI generation)
 */
export function generatePlaceholderImage(domainConfig: DomainConfig, score?: number, nodeCount?: number): string {
  ...
  const svg = `
    <svg width="512" height="512" xmlns="http://www.w3.org/2000/svg">
      ...
      ${score !== undefined ? `<text x="256" y="370" font-size="32" text-anchor="middle" fill="white" font-family="system-ui, -apple-system">${score}% Score</text>` : ''}
      ${nodeCount !== undefined ? `<text x="256" y="410" font-size="24" text-anchor="middle" fill="white" opacity="0.8" font-family="system-ui, -apple-system">${nodeCount} nodes</text>` : ''}
