...
export default function Navigation() {
  ...
  const navItems = [
    ...
    { name: 'Domains', href: '#domains', scroll: true },
    { name: 'Badges', href: '#badges', scroll: true },
  ...
  const NavItem = ({ item }: { item: typeof navItems[0] }) => {
    const commonClasses = "relative z-10 flex items-center rounded-md py-2 px-4 font-medium text-inherit transition-opacity duration-400 ease-out hover:opacity-80 cursor-pointer";
    if (item.href) {
      ...
      if (item.scroll) {
        return <a href={item.href} onClick={(e) => handleSmoothScroll(e, item.href || '#')} className={commonClasses}>{item.name}</a>;
      }
        ...
        <div className="pointer-events-auto w-full border-t border-border-light bg-background-secondary md:hidden">
          <nav className="flex flex-col space-y-1 p-4 text-foreground-primary">
            {navItems.map((item) => {
                ...
                if (item.scroll) {
                  return (
                    <a
                      ...
                      className="relative z-10 flex items-center rounded-md py-2 px-4 font-medium text-inherit transition-opacity duration-400 ease-out hover:opacity-80 cursor-pointer"
                    >
                      {item.name}
                    </a>
                  );
              ...
              return <button key={item.name} className="flex w-full items-center justify-between rounded-md p-2 text-left hover:bg-black/5">{item.name} <ChevronDown className="h-4 w-4 shrink-0"/></button>;
            })}
